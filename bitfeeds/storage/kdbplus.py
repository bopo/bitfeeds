from bitfeeds.storage.database import DatabaseStorage
...
class KdbPlusStorage(DatabaseStorage):
    """
    Kdb+ Client
    """
    @classmethod
    ...
    def connect(self, **kwargs):
        ...
        host = kwargs['host']
        port = kwargs['port']
        Logger.info(self.__class__.__name__, 'Kdb+ database storage is connecting to %s:%d' % (host, port))
    ...
    def select(self, table, columns=['*'], condition='', orderby='', limit=0, isFetchAll=True):
        ...
        if isinstance(select_ret, QKeyedTable):
            ...
            raise Exception("Unknown type (%s) in kdb storage select statement.\n%s" % (type(select_ret), select_ret))
...
if __name__ == '__main__':
    ...
    db_storage.create('test', ['c1', 'c2', 'c3', 'c4'], ['varchar(20)', 'int', 'decimal(8, 20)', 'int'], [0], False)
    ...
    Logger.info('test', db_storage.select('test', columns=['c2', 'c3'], condition='c1 >= "abc" and c2 > 1'))
    # Logger.info('test', db_storage.select('test', columns=['*'], orderby='c1 desc', limit=1))
    db_storage.delete('test', 'c1="abc"')
