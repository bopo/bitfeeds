...
class ZmqStorage(DatabaseStorage):
    """
    Zmq Client
    """
    def __init__(self):
    ...
    def connect(self, **kwargs):
        ...
        """
        addr = kwargs['addr']
        Logger.info(self.__class__.__name__, 'Zmq storage is connecting to %s' % addr)
        self.conn.bind(addr)
...
if __name__ == '__main__':
    ...
    db_storage = ZmqStorage()
    db_storage.connect(addr='ipc://test')

    for i in range(1, 100):
        db_storage.insert('test', ['c1', 'c2', 'c3', 'c4'], [], ['abc', i, 1.1, 5])
        time.sleep(1)
