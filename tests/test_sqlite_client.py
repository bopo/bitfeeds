...
import unittest
import os
from bitfeeds.storage.sqlite import SqliteClient
from bitfeeds.util import Logger

file_name = 'tests/test.sqlite'
...
class SqliteClientTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        Logger.init_log()
        cls.db_storage = SqliteClient()
        cls.db_storage.connect(path=file_name)
    ...
    def test_query(self):
        ...
        row = self.db_storage.select(table=table_name, columns=[columns[0]]+[columns[2]])
        self.assertEqual(len(row), 3)
        self.assertEqual(row[0][0], "20161026")
        ...
        # Negative case
        self.assertTrue(not self.db_storage.create(table_name, columns[1::], types))
        self.assertTrue(not self.db_storage.insert(table_name, columns, types, []))
